...
const countNumberOfChildren = (arr) => {
  return arr.reduce((numKids, character) => {
    if(character.hasOwnProperty('children')) {
      numKids += character.children.length;
    }
    return numKids;
  }, 0);
};
...
const extractStat = (statName, arr) => {
  return arr.reduce((matchingStat, val) => {
...
const extractChildren = (arr) => {
  let filtered = arr.filter(character => {
    return character.name.match(/^.*[aA].*$/)
  });
  return filtered.reduce((children, character) => {
    return character.children ? children.concat(character.children) : children;
  }, []);
};
