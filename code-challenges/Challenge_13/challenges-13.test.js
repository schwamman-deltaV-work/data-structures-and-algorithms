...
const onlyOddChars = (str) => {
  return str.split('').filter((character, i) => i % 2 === 1 ? true : false).join('');
};
...
const findEvery = (arr, target) => {
  return arr.length === arr.filter(string => string.includes(target)).length;
};
...
const unenrollBrook = (arr) => {
  return arr.map(array => array.filter(name => !name.includes('Brook')));
};
...
const sortByDay = (arr) => {
  return arr.reduce((schedule, day) => {
...
const characterByIndex = (arr) => {
  return arr.map((string, i) => string.substring(i, i + 1));
};
